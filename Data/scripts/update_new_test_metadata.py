...
# after referencing mapping and hierarchy, and filtering against valid target classes.
...
# --- Configuration ---
INPUT_CSV = "../metadata/final_test/final_test_airliners_metadata.csv"
OUTPUT_CSV = "../metadata/final_test/final_test_airliners_metadata_updated.csv"
AIRLINE_YAML = "../class_definitions/airline_mapping.yaml"
AIRCRAFT_YAML = "../class_definitions/aircraft_hierarchy.yaml"

# Valid class lists
VALID_AIRLINES_CSV = "../metadata/counts_airlines_merged_trimmed.csv" # Expects column: 'Airline'
VALID_VARIANTS_CSV = "../metadata/counts_variants_trimmed.csv"        # Expects column: 'Variant'

def load_valid_classes():
    """Loads the specific lists of valid variants and airlines from their respective CSVs."""
    valid_variants, valid_airlines = set(), set()
    
    if os.path.exists(VALID_VARIANTS_CSV):
        df_v = pd.read_csv(VALID_VARIANTS_CSV)
        if 'Variant' in df_v.columns:
            valid_variants = set(df_v['Variant'].dropna().astype(str).unique())
            
    if os.path.exists(VALID_AIRLINES_CSV):
        df_a = pd.read_csv(VALID_AIRLINES_CSV)
...
def update_dataset():
    ...
    print("Applying YAML taxonomies and valid class filters...")
    ...
    # 1. Update Airline Alias to Merged Airline, then check validity
    df['airline'] = df['airline'].map(airline_lookup).fillna(df['airline'])
    df['airline'] = df['airline'].apply(lambda x: x if str(x).strip() in valid_airlines else "OTHERS")

    # 2. Read from 'aircraft_model' and CREATE the 'aircraft_variant' column
    df['aircraft_variant'] = df['aircraft_model'].map(model_to_variant).fillna(df['aircraft_model'])

    # 3. Add Family and Manufacturer columns based on the TRUE mapped variant
    # (We do this BEFORE masking to "OTHERS" so we don't lose the family/manufacturer data)
    ...
    # 4. Check validity of the Variant. If invalid, mask it to "OTHERS"
    df['aircraft_variant'] = df['aircraft_variant'].apply(lambda x: x if str(x).strip() in valid_variants else "OTHERS")
